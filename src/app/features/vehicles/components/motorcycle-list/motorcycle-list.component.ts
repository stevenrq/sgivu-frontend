...
export class MotorcycleListComponent implements OnInit {
  ...
  readonly filters = signal<MotorcycleSearchFilters>(this.createFilterState());
  ...
  protected clearSearchTerm(): void {
    this.filters.update((f) => ({ ...f, plate: '' }));
    this.applyFilters();
  }

  /** Aplica un parche parcial sobre los filtros (bindings del panel avanzado). */
  protected patchFilters(patch: Partial<MotorcycleSearchFilters>): void {
