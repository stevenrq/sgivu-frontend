...
describe('VehicleImageService', () => {
  ...
  describe('uploadToPresignedUrl()', () => {
    it('Debe subir archivo a URL presignada y retornar cuerpo de respuesta', async () => {
    ...
    it('Debe manejar respuesta de error HTTP desde URL presignada', async () => {
      const url = 'https://s3.example.com/presigned-url';
      ...
      service.uploadToPresignedUrl(url, file, contentType).subscribe({
        next: () => expect.unreachable('should error'),
        error: (err) => {
    ...
    it('Debe manejar error de red en fetch', async () => {
      const url = 'https://s3.example.com/presigned-url';
      const file = new File(['test'], 'test.jpg', { type: 'image/jpeg' });
      const contentType = 'image/jpeg';
  ...
  describe('deleteImage()', () => {
    ...
    it('Debe manejar error de eliminación (404 imagen no encontrada)', async () => {
      const vehicleId = 1;
      const imageId = 999;
