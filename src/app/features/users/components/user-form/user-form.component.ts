...
/** Nombre del rol asignado por defecto a los usuarios nuevos (tal como lo devuelve el backend). */
...
export class UserFormComponent implements OnInit {
  ...
  /** Identificadores de los roles seleccionados en el formulario. */
  protected readonly selectedRoleIds = signal<Set<number>>(new Set<number>());
  /** Indica si el formulario edita el perfil del propio usuario autenticado. */
  protected readonly isSelfEdit = signal(false);
  ...
  ngOnInit(): void {
    this.isSelfEdit.set(this.route.snapshot.data?.['selfEdit'] === true);
    ...
    this.roleService
      ...
      .subscribe((roles) => {
        ...
        this.defaultRole =
          roles.find((role) => role.name === DEFAULT_ROLE_NAME) ?? null;
        if (
          !this.isEditMode() &&
          !this.selectedRoleIds().size &&
          this.defaultRole
        ) {
          this.selectedRoleIds.set(new Set([this.defaultRole.id]));
        }
      });
  ...
  private resetToCreateMode(): void {
    ...
    this.selectedRoleIds.set(
      this.defaultRole ? new Set([this.defaultRole.id]) : new Set<number>(),
    );
  ...
  private rolesToSend(): Role[] {
    if (this.isSelfEdit()) {
      return [...this.currentRoles];
    }

    const selected = this.availableRoles().filter((role) =>
      this.selectedRoleIds().has(role.id),
    );
    if (selected.length) {
      return selected;
    }

    if (this.isEditMode() && this.currentRoles.length) {
      return [...this.currentRoles];
    }
