...
export class CompanyListComponent implements OnInit {
  ...
  readonly filters = signal<CompanySearchFilters & { enabled?: FilterEnabled }>(
    this.createFilterState(),
  );

  private activeFilters: CompanySearchFilters | null = null;
  private readonly queryParams = signal<Params | null>(null);
  ...
  protected onSearchValueChange(value: string): void {
    this.filters.update((f) => ({ ...f, companyName: value }));
  }

  protected clearSearchTerm(): void {
    this.filters.update((f) => ({ ...f, companyName: '' }));
    this.applyFilters();
  }

  /** Aplica un parche parcial sobre los filtros (bindings del panel avanzado). */
  protected patchFilters(
    patch: Partial<CompanySearchFilters & { enabled?: FilterEnabled }>,
  ): void {
  ...
  ngOnInit(): void {
    combineLatest([this.route.paramMap, this.route.queryParamMap])
      ...
      .subscribe(([params, query]) => {
        ...
        this.filters.set(filterInfo.uiState);
        this.activeFilters = filterInfo.filters;
        this.queryParams.set(filterInfo.queryParams);
  ...
  protected clearFilters(): void {
    this.filters.set(this.createFilterState());
