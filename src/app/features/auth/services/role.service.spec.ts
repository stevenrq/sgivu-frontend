...
describe('RoleService', () => {
  ...
  describe('addPermissions()', () => {
    ...
    it('Debe propagar error en falla de POST', async () => {
      service.addPermissions(1, ['perm']).subscribe({
        next: () => expect.unreachable('No debería tener éxito'),
        error: (err) => {
  ...
  describe('updatePermissions()', () => {
    ...
    it('Debe propagar error en falla de PUT', async () => {
  ...
  describe('removePermissions()', () => {
    ...
    it('Debe propagar error en falla de DELETE', async () => {
  ...
  describe('estado de roles (signal)', () => {
    it('Debe poblar el signal de roles tras findAll()', () => {
      const mockRoles = [buildRole({ id: 1, name: 'ADMIN' })];

      service.findAll().subscribe();
      httpMock.expectOne(`${environment.apiUrl}/v1/roles`).flush(mockRoles);

      expect(service.roles()).toEqual(mockRoles);
    });

    it('Debe sincronizar el rol actualizado en el signal tras updatePermissions()', () => {
      const initial = buildRole({ id: 2, name: 'USER' });
      service.findAll().subscribe();
      httpMock.expectOne(`${environment.apiUrl}/v1/roles`).flush([initial]);
      ...
      service.updatePermissions(2, ['user:read']).subscribe();
      httpMock
        .expectOne(`${environment.apiUrl}/v1/roles/2/permissions`)
        .flush(updated);

      expect(service.roles()).toEqual([updated]);
    });
  });
});
