...
export class PurchaseSaleCreateComponent implements OnInit {
  ...
  readonly contractFormGroup = signal<FormGroup<ContractFormControls>>(
    buildContractFormGroup(this.fb),
  );
  readonly vehicleFormGroup = signal<FormGroup<VehicleFormControls>>(
    buildVehicleFormGroup(this.fb),
  );
  ...
  // Tipo de contrato reactivo: se re-suscribe al `valueChanges` cada vez que el
  // FormGroup se reconstruye. Reactivo bajo zoneless tanto por evento del usuario
  // como desde el callback async de query params.
  private readonly contractType = toSignal(
    toObservable(this.contractFormGroup).pipe(
      switchMap((group) =>
        group.controls.contractType.valueChanges.pipe(
          startWith(group.controls.contractType.value),
        ),
      ),
    ),
    { initialValue: ContractType.PURCHASE },
  );

  readonly isPurchaseType = computed(
    () => this.contractType() === ContractType.PURCHASE,
  );

  readonly isSaleType = computed(
    () => this.contractType() === ContractType.SALE,
  );
  ...
  onContractTypeChange(type: ContractType): void {
    ...
    this.contractFormGroup().controls.salePrice.reset(null, {
      emitEvent: false,
    });
    this.contractFormGroup().controls.vehicleId.reset(null, {
      emitEvent: false,
    });
  ...
  resetContractForm(keepSelections = false): void {
    ...
    this.contractFormGroup.set(
      buildContractFormGroup(this.fb, selectedContractType),
    );
    this.vehicleFormGroup.set(
      buildVehicleFormGroup(this.fb, selectedVehicleType),
    );
    this.purchasePriceInput.set('');
    this.salePriceInput.set('');
